...
def execute_single_request(args: argparse.Namespace, logger: logging.Logger) -> RequestResult:
    ...
    # Setup retry config
    retry_config = RetryConfig(max_retries=args.retries, initial_delay=1.0)
...
def execute_test_suite(args: argparse.Namespace, logger: logging.Logger) -> list[RequestResult]:
    ...
    # Setup retry config
    retry_config = RetryConfig(max_retries=defaults.get("retries", 3), initial_delay=1.0)
    ...
    with Progress(console=console) as progress:
        ...
        for test in tests:
            ...
            # Create request config
            config = RequestConfig(
                method=method, url=url, headers=headers, params=params, body=body, timeout=timeout
            )
...
[bold]Category:[/bold] {diagnosis.category}"""
    ...
    if result.response_body:
        ...
        console.print(Panel(body_preview, title="Response Body", border_style="blue"))
