...
class RetryHandler:
    ...
    def should_retry(
        self, status_code: int | None = None, exception: Exception | None = None
    ) -> bool:
        """
...
def with_retry(func: Callable, retry_config: RetryConfig | None = None, *args, **kwargs) -> Any:
    """
    ...
    while True:
        try:
            ...
            if handler.should_retry(exception=e):
                ...
                print(
                    f"Retry {handler.get_retry_count()}/{handler.config.max_retries} "
...
def calculate_backoff_delay(
    attempt: int, initial_delay: float = 1.0, max_delay: float = 32.0
) -> float:
    """
